...
function enableSongLyricsPool(songs){
	...
	songs.forEach(song => {
		...
		const button = document.createElement("button");
		const text = document.createTextNode(lyrics[0]);
		button.appendChild(text);
		const span = document.createElement("span");
		for(let lyric of lyrics) {
			span.innerHTML += `${lyric}<br>`;
		}
...
const takahashiYuOpenWorld = [`
Open World…
[Verse 1]…
期待すればするほどに裏切られ ⼀⾔多いだけで叩きのめされ…
...
素晴らしいかもしれない 今⽇を始めよう…
`];
enableSongLyricsPool(takahashiYuOpenWorld);
