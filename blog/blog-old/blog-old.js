...
/**
 * 
 * @param {Object[]} allPostsObject 
 */
function blogList(allPostsObject) {
	const bpTitles = [allPostsObject.length];
	allPostsObject.forEach(object, index => {
		bpTitles[index] = object.title;
	});
	return bpTitles;
}
...
( async () => {
	...
	enableBlogSelect();
	const titlesList = blogList(blogPosts);
	console.log(titlesList[1]);
